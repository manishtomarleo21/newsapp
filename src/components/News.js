...
export class News extends Component {
    articles = [
        ...
        },
        {
            ...
            "content": "Washington: American children's mental health crisis has gotten worse during the Covid-19 pandemic, according to two new studies published by the US Centers for Disease Control and Prevention (CDC).\r… [+1381 chars]"
        },
        {
            "source": {
                "id": null,
            ...
            "content": "Out-of-form India batters Cheteshwar Pujara and Ajinkya Rahane have both been dropped from India's squad for the upcoming Test series against Sri Lanka. Both veteran batters have been going through a… [+1625 chars]"
        },
        {
            "source": {
                "id": null,
            ...
            "content": "PUBG New State, which recently changed its name to just New State Mobile, has gained immense popularity among gamers globally in a short time. After the ban of Garena Free Fire in India, New State Mo… [+2225 chars]"
        },
        {
            "source": {
                "id": null,
        ...
        },
        {
            ...
            "content": "Apple iPhone 14 is one of the most awaited smartphones and fans want to know about even the smallest of updates related to it. Though the launch of the iPhone 14 is not yet officially confirmed, but … [+2281 chars]"
        },
        {
            "source": {
                "id": null,
        ...
        },
        {
            ...
            "title": "UP polls: BJP slams Akhilesh for ‘silence’ over sentencing of Ahmedabad blasts convicts - Hindustan Times",
            ...
            "content": "A day before Uttar Pradesh votes in the third phase, leaders of the ruling Bharatiya Janata Party (BJP) on Saturday launched an all-out attack on Samajwadi Party (SP) chief Akhilesh Yadav, claiming t… [+2700 chars]"
        },
        {
            "source": {
                "id": null,
            ...
            "author": "Livemint",
            "title": "Future Retail pays nearly ₹104.55 cr interest on dollar bond in grace period - Mint",
            ...
            "content": "Future Retail Ltd (FRL) has made a payment of USD 14 million (around 104.55 crore) as interest due on the dollar-denominated notes (USD Notes) listed on the Singapore Stock Exchange, the debt-ridden … [+2083 chars]"
        },
        {
            "source": {
                "id": null,
        ...
        },
        {
            ...
            "content": "Punjab polls: Charanjit Singh Channi had visited a temple with Sidhu Moosewala in Manasa yesterday\r\nManasa: Punjab Chief Minister Charanjit Singh Channi and singer Shubhdeep Singh Sidhu aka Sidhu Moo… [+1796 chars]"
        },
        {
            "source": {
            ...
            "content": "First in Karnataka: Mysuru college cancels uniform rule to allow hijabsIn the first punitive action against students seeking entry into colleges despite the interim order of the Karnataka high court … [+170 chars]"
        },
        {
            "source": {
                "id": null,
            ...
            "description": "Prime Minister Narendra Modi said the construction of the Bio-CNG plant is a commendable effort to ensure “cleaner urban spaces under the principle of Waste to Wealth.” | Latest News India",
        ...
        },
        {
            ...
            "description": "Assembly Elections 2022 LIVE: In a major setback to Arvind Kejriwal-led Aam Aadmi Party (AAP) days ahead of the Punjab Assembly elections, Union Home Minister Amit Shah has assured state chief minister Charanjit Singh Channi that he would \"personally look int…",
        ...
        },
        {
            ...
            "content": "Updated connected car tech in the new Baleno will allow owners to issue commands through paired Alexa devices.In the build up to its launch on February 23, Maruti Suzuki has revealed more details on … [+2838 chars]"
        },
        {
            "source": {
                "id": null,
            ...
            "content": "Virat Kohli and Rishabh Pant have been given rest for the third T20 International against the West Indies scheduled in Kolkata on Sunday. The duo will not be playing in the three-match T20 series aga… [+2619 chars]"
        },
        {
            "source": {
                "id": null,
            ...
            "content": "Satellite images have shown increased presence of Russian forces along the Ukrain border.\r\nThe tension along the Ukrainian border had been brewing for months, but escalated in recent months after Rus… [+5127 chars]"
        },
        {
            "source": {
                "id": null,
    ...
    async updateNews(){
        const url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apikey=b891761294164c5ea9c739ecf0c79486&page=${this.state.page}&pagesize=${this.props.pageSize}`;
    ...
    async componentDidMount(){
        // let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apikey=b891761294164c5ea9c739ecf0c79486&page=1&pagesize=${this.props.pageSize}`;
        // this.setState({loading:true})
        // let data = await fetch(url);
        // let parsedData = await data.json()
        // // console.log(parsedData);
    ...
    handleNextClick = async ()=>{
        ...
        //     let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apikey=b891761294164c5ea9c739ecf0c79486&page=${this.state.page+1}&pagesize=${this.props.pageSize}`;
        //     this.setState({loading:true})
        //     let data = await fetch(url);
        //     let parsedData = await data.json()
        //     // console.log(parsedData);
    ...
    render() {
        return (
            <div className='container my-3'>
                <h2 className='text-center' >News Monkey - Top {this.capitalize(this.props.category)} Headlines</h2>
                ...
                <button disabled={this.state.page<=1} type="button" className="btn btn-outline-dark mx-4" onClick={this.handlePrevClick}>&larr; Previous</button>
                <button type="button" disabled={this.state.page + 1 > Math.ceil(this.state.totalResults/this.props.pageSize)} className="btn btn-outline-info" onClick={this.handleNextClick}>&rarr;  Next</button>
                </div>
            </div>
        )
    }
